...
| KL III §3.2 (partial) | `END_U(1_λ)` is commutative (Eckmann–Hilton); every real or fake bubble lies in the image of `Π_λ → END(1_λ)` (3.25); every closed diagram without crossings lies in that image (simply-laced data); the image is nonnegatively graded with degree-0 part `k`, and the conclusions of Cor. 3.7 follow from Prop. 3.6; the map `ϕ_{ν,λ} : R(ν) ⊗ Π_λ → END(E_ν1_λ)` (3.30). Prop. 3.6 and Prop. 3.10 are reduced to one statement (the diagrammatic Lemma 3.9: every upward-to-upward diagram lies in the span of upward diagrams with bubbles), which is not yet proved; the Markov lemmas (left and right partial traces preserve that span) and Prop. 3.6 for iterated right traces of upward diagrams are proved | `KL3.Diagram.bubMap`, `KL3.Diagram.crossFree_isBub`, `KL3.Diagram.prop36_of_reduction`, `KL3.Diagram.cor37_gdim_le_pi`, `KL3.Diagram.phi`, `KL3.Diagram.prop36_of_upSpanDiag`, `KL3.Diagram.prop310_of_upSpanDiag`, `KL3.Diagram.ptrLast_upward_mem_upSpan`, `KL3.Diagram.ptrFirst_mem_upSpan`, `KL3.Diagram.trR_dg_isBub` |
...
| KL III §3.6, Props. 3.27, 3.28 (integral, partial) | associativity and units of the product on `K_0(U̇)` (an idempotented `ℤ[q,q⁻¹]`-algebra); divided-power classes `[E_d 1_λ]` with `[E_{i^a}] = [a]_i! [E_{i^{(a)}}]`; the defining relations of `_A U̇` hold in `K_0(U̇)` up to `ℤ[q,q⁻¹]`-torsion, and `γ : _A U̇ → K_0(U̇)` on each block assuming `K_0` torsion free (Krull–Schmidt in `U̇` is not formalized); `γ` intertwines `ω` with `[ω̃]` (exactly) and `ψ` with `[ψ̃]` (over `ℚ(q)`) | `GradedBicat.K0U.mul_assoc`, `KL3.Diagram.dpC_relation`, `KL3.Diagram.gammaUA`, `KL3.Diagram.omegaK0_dpC_exact`, `KL3.Diagram.gammaQ'_omega`, `KL3.Diagram.gammaQ'_psi` |
