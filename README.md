...
| KL I Prop. 2.19 | `Res_{ν,ν'} P_k ≅ ⊕ P_i ⊗ P_j` over the ways of writing `k` as a shuffle of `i` and `j`, for left and for right modules (ungraded), and the right-module formula with the shifts `deg(i, j, s)` (graded) | `KLR.KLRAlgebra.resProjEquiv`, `KLR.KLRAlgebra.resProjEquivRight`, `KLR.KL1.resProjGradedEquivRight` |
| KL I Lemmas 3.13, 3.14 | for `n ≤ ε_i(M)` the simple submodule of `Δ_{i^n} M` is `ẽ_i^n M ⊠ L(i^n)`, and `hd Ind(N ⊠ L(i^n)) ≅ f̃_i^n N`; for `M` simple, the socle of `e_{i^n} M` is that submodule, isomorphic to `(ẽ_i^n M)^{⊕ n!}` as an `R(μ)`-module; graded: the socle is homogeneous and isomorphic to `⊕_u HW(S){2|u|}` over the staircase monomials, so `gdim soc = q^{n(n-1)/2} [n]! gdim HW(S)` (the printed shift `{-n(n-1)/2}` corresponds to normalizing `ẽ_i^n M` as the top summand `HW(S){n(n-1)}`) | `KLR.KLRAlgebra.lemma_3_13_soc`, `KLR.KLRAlgebra.lemma_3_13_hd`, `KLR.KLRAlgebra.lemma_3_14`, `KLR.KLRAlgebra.lemma_3_14_graded`, `KLR.KLRAlgebra.gdimPoly_socle_kl` |
...
| KL III §3.2.3 (Prop. 3.12, (3.68), Cor. 3.13) | the elements of the proposed spanning sets `B_{𝐢,𝐣,λ}` (pairing, dots per strand, bubble monomial) in each degree are counted by `π⟨E_𝐢1_λ, E_𝐣1_λ⟩` (Prop. 3.12); a spanning family is a basis iff the dimension count holds (3.68); Cor. 3.13 and the equivalence of KL's basis definition of nondegeneracy with the dimension form are stated under a hypothesis `Prop311` which, as formalized, is equivalent to the inequality of Cor. 3.13 itself and is weaker than KL's Prop. 3.11 (spanning by the explicit sets `B_{𝐢,𝐣,λ}`, not yet proved) | `KL3.Diagram.prop_3_12`, `KL3.Diagram.finrank_eq_iff_linearIndependent`, `KL3.Diagram.cor_3_13`, `KL3.Diagram.calculusNondeg_iff` |
| KL III Prop. 3.11 (positive sequences), Cor. 3.13 | for sequences of `E`'s, the explicit sets `B_{𝐢,𝐣,λ}` (reduced-word crossings, dots at the bottom, bubble monomials on the right) span each graded piece of `HOM_U(E_𝐢1_λ, E_𝐣1_λ)`; Cor. 3.13 for positive sequences; nondegeneracy for positive sequences is equivalent to linear independence of these sets, and then `γ` is bijective | `KL3.Diagram.prop_3_11_positive`, `KL3.Diagram.cor_3_13_positive`, `KL3.Diagram.positiveNondeg_iff`, `KL3.Diagram.gammaUA'_bijective_of_linearIndependent` |
...
| CL §2.6, Remark (3) on p. 8; KL III §4.2.1 and erratum | rescaling isomorphisms: a unit scaling of dots, upward crossings and cups (with the cup/cap scalars constrained by the bubble relations, downward crossings forced by `Q`-cyclicity) gives an isomorphism `U_Q(𝔤) ≅ U_{Q'}(𝔤)` with `Q'_{ij}(u, v) = (c_{ij} c_{ji})^{-1} Q_{ij}(a_i u, a_j v)`, `r'_i = r_i (a_i c_{ii})^{-1}`; hence every `U_Q` is isomorphic to one with `r = 1`, a single-vertex `U_Q` is isomorphic to KL III's `U`, and so is any `U_Q` with balanced scalars and `s = 0` | `KL3.Diagram.CL.RescaleDatum.equiv`, `KL3.Diagram.CL.RescaleDatum.scL_relationCL`, `KL3.Diagram.CL.equivWithRone`, `KL3.Diagram.CL.equivSingleVertexPres`, `KL3.Diagram.CL.equivOfBalanced` |
| KL III §4.2.1, erratum p. 99 | the isomorphism `Σ : U(sl_n) ≅ U_Q(sl_n)` with `t_{ij} = i - j` (the corrected `U→(sl_n)`): dots and upward crossings are scaled as in KL III, cups and caps as in the erratum; the downward crossings carry weight-dependent signs forced by cyclicity | `KL3.Diagram.CL.Sln.sigmaEquiv`, `KL3.Diagram.CL.Sln.sigmaEquivPres` |
| CL §1.2, §2.1.2 | graded additive `k`-linear 2-categories (Mathlib bicategories with preadditive, linear Hom categories with a shift `⟨1⟩` commuting with whiskering), strong 2-representations of `sl_2` along a weight string, and `Q`-strong 2-representations of `𝔤` (Def. 1.2) | `TwoRep.GradedBicategory`, `TwoRep.StrongSl2`, `TwoRep.QStrong` |
| CL §3.1 | cancellation `A ⊕ B ≅ A ⊕ C ⇒ B ≅ C` in Krull–Schmidt categories, and `A ⊗ V ≅ B ⊗ V ⇒ A ≅ B` for `V ≠ 0` graded when no indecomposable is isomorphic to a nonzero shift of itself (as stated in CL, without these hypotheses, the second law fails, e.g. for `V = 0`); bricks, `A`-rank | `TwoRep.cancel_biprod`, `TwoRep.cancel_shiftSum`, `TwoRep.IsBrick`, `TwoRep.rk` |
...
`_{…i^{(2)}j…}P ⊕ _{…ji^{(2)}…}P`. In KL I §2.2, `L(i^m)` is nonzero in degrees `0, 2, …, m(m−1)`
(with `deg x = 2`), not `m(m−1)/2`.
