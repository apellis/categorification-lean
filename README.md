...
| KL III §3.2 (partial) | `END_U(1_λ)` is commutative (Eckmann–Hilton); every real or fake bubble lies in the image of `Π_λ → END(1_λ)` (3.25); every closed diagram without crossings lies in that image (simply-laced data); the image is nonnegatively graded with degree-0 part `k`, and the conclusions of Cor. 3.7 follow from Prop. 3.6; the map `ϕ_{ν,λ} : R(ν) ⊗ Π_λ → END(E_ν1_λ)` (3.30). The reduction of closed diagrams with crossings (Prop. 3.6 in general) is not yet formalized | `KL3.Diagram.bubMap`, `KL3.Diagram.crossFree_isBub`, `KL3.Diagram.prop36_of_reduction`, `KL3.Diagram.cor37_gdim_le_pi`, `KL3.Diagram.phi` |
| KL III §3.3 (partial) | the symmetry 2-functors `ψ̃` (reflection in the x-axis, `U → U^co`, fixing all bubbles) and `σ̃` (reflection in the y-axis with `λ ↦ -λ` and a sign on `ii`-crossings, `U → U^op`, exchanging clockwise and counterclockwise bubbles); `ψ̃² = 1`, `σ̃² = 1`, `σ̃ψ̃ = ψ̃σ̃`; both preserve degrees. `ω̃` and `τ̃` are not yet constructed (in (3.45), `ω̃σ̃ = ω̃σ̃` should read `ω̃σ̃ = σ̃ω̃`) | `KL3.Diagram.psiU`, `KL3.Diagram.sigU`, `KL3.Diagram.psiU_psiU`, `KL3.Diagram.sigU_sigU`, `KL3.Diagram.psiU_sigU` |
| KL III §3.6 (partial), Prop. 3.27 over `ℚ(q)` (conditional) | the split Grothendieck group `K_0(U̇(λ, μ))` (`q` = shift), the product induced by composition; in `K_0`: the `EF`/`FE` relations with quantum integers, `[E_i F_j] = [F_j E_i]` (`i ≠ j`), `[E_i^m] = [m]_i! [E_{i^{(m)}}]`, the upward Serre relation, and a sorting lemma (K_0 form of Lemma 3.38); the map `γ : U̇1_λ → ℚ(q) ⊗ K_0(U̇)` on each block, assuming the downward Serre relation in `K_0` as an explicit hypothesis | `SplitK0`, `GradedBicat.K0U.mul`, `KL3.Diagram.eC_EF`, `KL3.Diagram.eC_pow`, `KL3.Diagram.eC_serre`, `KL3.Diagram.gammaQ` |
